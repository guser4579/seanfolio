...
const REPO = 'seanfolio';
// 5 minutes: the counter tracks reality closely without hammering the API
// (a token allows 5,000 requests/hour; this uses a tiny fraction of it)
const REVALIDATE = 300;

function ghHeaders() {
  const h = { 'User-Agent': 'seanforquer.com code pulse' };
...
// all-time contribution count (commits, PRs, issues, reviews - the number
// GitHub's own graph shows), displayed under the "commits" label per Sean.
// contributionsCollection is windowed to one year, so sum a per-year alias
// query from account creation to now (two requests total, cached an hour).
async function fromGraphql(token) {
  ...
  for (let y = firstYear; y <= thisYear; y++) {
    aliases.push(
      `y${y}: contributionsCollection(from: "${y}-01-01T00:00:00Z", to: "${y + 1}-01-01T00:00:00Z") { contributionCalendar { totalContributions } }`
    );
  }
  const totals = await gql(token, `{ user(login: "${USER}") { ${aliases.join(' ')} } }`);
  const total = Object.values(totals.user).reduce(
    (sum, v) => sum + (v?.contributionCalendar?.totalContributions || 0),
    0
  );
...
// "august 2026" for the last commit that touched a repo path, or null.
// Dates that never lie: derived from history, not typed by hand.
export async function getFileUpdatedLabel(path) {
  try {
    const res = await fetch(
      `https://api.github.com/repos/${USER}/${REPO}/commits?path=${encodeURIComponent(path)}&per_page=1`,
      { headers: ghHeaders(), next: { revalidate: REVALIDATE } }
    );
    if (!res.ok) throw new Error(String(res.status));
    const iso = (await res.json())?.[0]?.commit?.committer?.date;
    if (!iso) throw new Error('no commits for path');
...
// a thought's display date: git-derived for entries flagged gitDated
// (evolving pieces like the ethos list), the hand-written date otherwise
export async function thoughtDate(t) {
  if (!t.gitDated) return t.date;
  const label = await getFileUpdatedLabel(t.gitDated);
  return label ? `last updated ${label}` : t.date;
}

// the site's own commit count - its version number. Read from the Link
// pagination header of a 1-per-page commit listing.
